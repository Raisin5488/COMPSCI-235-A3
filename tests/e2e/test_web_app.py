...
def test_index(client):
    ...
    assert b'COMPSCI 235-flix' in response.data


def test_login_get_watch_list(client):
    response = client.get('/watch_list')
    assert response.headers['Location'] == 'http://localhost/authentication/login'


def test_login_add_to_watch_list(client):
    response = client.post('/add_watch_list')
    assert response.headers['Location'] == 'http://localhost/authentication/login'


def test_login_add_review(client):
    response = client.post('/add_watch_list')
    assert response.headers['Location'] == 'http://localhost/authentication/login'
